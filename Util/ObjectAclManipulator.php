...
namespace Sonata\DoctrinePHPCRAdminBundle\Util;
...
class ObjectAclManipulator extends BaseObjectAclManipulator
{
    ...
    public function batchConfigureAcls(OutputInterface $output, AdminInterface $admin, UserSecurityIdentity $securityIdentity = null)
    {
        ...
        $dm = $admin->getModelManager()->getDocumentManager($admin->getClass());

        $qb = $dm->createQueryBuilder();
        $qb->from()->document($admin->getClass(), 'o') ;
        ...
        try {
            ...
            foreach ($qb->getQuery()->execute() as $document) {
                $objectIds[]      = ObjectIdentity::fromDomainObject($document);
